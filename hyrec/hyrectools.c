...
Version: January 2015 (added cubic interpolation for non-evenly spaced table)
*****************************************************************************/
...
double rec_interpol_G(double x, double *xtab, double *ytab, unsigned int Nx) {
  ...
  if (Nx < 4) {
    fprintf(stderr, "Error: rec_interpol_G: Table needs to be of dimension 4 at least\n");
    exit(1);
  }
  if (xtab[0] > xtab[1]) {
     fprintf(stderr, "Error: rec_interpol_G: Array does not seem to be increasing.\n");
     exit(1);
  }
  if (x < xtab[0] || x >= xtab[Nx-1]) {
    fprintf(stderr, "Error: rec_interpol_G: x-value out of range.\n");
     exit(1);
  }
