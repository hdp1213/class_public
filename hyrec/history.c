...
int rec_get_xe_next1_He(REC_COSMOPARAMS *cosmo, double z_in, double *xHeII,
  ...
  double dxHeIIdlna_p, dxHeIIdlna_m;

  H = rec_HubbleRate(cosmo, z_in);

  xH1        = rec_saha_xH1(*xHeII, cosmo->nH0, cosmo->T0, z_in, cosmo->fsR, cosmo->meR);
  class_call(rec_helium_dxHeIIdlna(xH1, *xHeII, cosmo->nH0, cosmo->T0, cosmo->fHe, H,
                                   z_in, cosmo->fsR, cosmo->meR, &dxHeIIdlna, error_message),
             error_message,
             error_message);
  ...
  if (*post_saha == 1) {
      ...
      class_call(rec_helium_dxHeIIdlna(xH1_p, xHeIISaha+Dxe, cosmo->nH0, cosmo->T0,
                                       cosmo->fHe, H, z_out, cosmo->fsR, cosmo->meR,
                                       &dxHeIIdlna_p, error_message),
                 error_message,
                 error_message);
      class_call(rec_helium_dxHeIIdlna(xH1_m, xHeIISaha-Dxe, cosmo->nH0, cosmo->T0,
                                       cosmo->fHe, H, z_out, cosmo->fsR, cosmo->meR,
                                       &dxHeIIdlna_m, error_message),
                 error_message,
                 error_message);
      DdxHeIIdlna_Dxe  = (dxHeIIdlna_p - dxHeIIdlna_m)/2./Dxe;
...
int rec_xH1_stiff(int model, REC_COSMOPARAMS *cosmo, double z, double xHeII, double *xH1,
  ...
  *xH1 = xeq + dxeq_dlna/Gamma;
  ...
  class_test((*xH1 < 0.) || (*xH1 > 1.) || (*xH1 != *xH1),
             error_message,
             "At z = %f, xH1 = %E", z, xH1);
...
int rec_build_history(int model, double zstart, double zend,
  ...
  ********/
  ...
  for(; iz <= iz_rad_0; iz++) {
    class_call(rec_get_xe_next1_He(cosmo, z, &xHeII, dxHeIIdlna_prev, &quasi_eq, error_message),
               error_message,
               error_message);
    z             = (1.+zstart)*exp(-DLNA*iz) - 1.;
...
int hyrec_allocate(HYREC_DATA *data, double zmax, double zmin, short read_atomic_files, ErrorMsg error_message) {
  ...
  if (read_atomic_files == _TRUE_) {
    class_call(allocate_and_read_atomic(data->atomic, error_message),
               error_message,
               error_message);
  }
...
int hyrec_compute(HYREC_DATA *data, int model,
  ...
  class_call(rec_build_history(model, data->zmax, data->zmin, data->cosmo, data->atomic,
                    data->rad, data->pbh, data->xe_output, data->Tm_output, error_message),
             error_message,
             error_message);
