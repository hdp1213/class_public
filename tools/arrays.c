...
/**
 * Interpolate a 2D array using (bi)linear interpolation where the
 * x-axis has monotonically increasing values and the y-axis has
 * monotonically decreasing values (decy).
 *
 * @param array     Input: 2D array of form f(x,y) to interpolate.
 * @param x_values  Input: Values of x-axis (increasing)
 * @param x_size    Input: Size of x-axis
 * @param y_values  Input: Values of y-axis (decreasing)
 * @param y_size    Input: Size of y-axis
 * @param x         Input: Value of x to interpolate at
 * @param y         Input: Value of y to interpolate at
 * @param result    Output: The interpolation result.
...
int array_interpolate_2d_array_bilinear_decy(
                               ...
                               double *result,
                               ErrorMsg errmsg) {
  /* Variables take after the bilinear interpolation Wikipedia page */
  int x1_ind,x2_ind;
  double x1,x2;
  int y1_ind,y2_ind;
  double y1,y2;
  ...
  /* TODO(harry): check that x and y are in the axes bounds */

  /* Check that x and y aren't already axis points (to skip interpolation) */
  /* NB: interpolation already gives correct results in such a case */

  /* Search x_values for x1,x2 at which to interpolate between */
  x1_ind = 0;
  x2_ind = 0;
  while (x >= x_values[x2_ind]) {
    x1_ind = x2_ind;
    x2_ind++;
  }
  x1 = x_values[x1_ind];
  ...
  /* Search y_values for y1,y2 at which to interpolate between */
  y1_ind = 0;
  y2_ind = 0;
  while (y <= y_values[y2_ind]) {
    y1_ind = y2_ind;
    y2_ind++;
  }
  y1 = y_values[y1_ind];
  ...
  f11 = array[y1_ind*x_size+x1_ind];
  f21 = array[y1_ind*x_size+x2_ind];
  f12 = array[y2_ind*x_size+x1_ind];
  f22 = array[y2_ind*x_size+x2_ind];
